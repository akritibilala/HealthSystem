...
public class AlertHealthSupporterPanel extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AlertHealthSupporterPanel frame = new AlertHealthSupporterPanel();
	 ...
	 * Create the frame.
	 */
	public AlertHealthSupporterPanel() {
		...
		Object columnNames[] = { "Alert Id", "Patient Id","Patient Name","Alert Type", "Alert Message", "Alert Date","Observation"};
		...
		for(Alert auth: alertList)
		{
			...
			rowData[i][2] = auth.getPatient().getName();
			rowData[i][3] = auth.getType();
			rowData[i][4] = auth.getAlertMessage();
			rowData[i][5] = auth.getDate();
			rowData[i][6] = auth.getObsType().getType();
			i++;
		}	
